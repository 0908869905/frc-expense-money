...
> 2026-07-03 ~ 07-05，由 Claude Fable 5 完成。
> 分支：`redesign/ui-v2`（**未動 main、未部署**）

---

## 🏮 最終定案：極簡日式「和紙と墨」（07-05）

用戶選定日式版為正式方向。`redesign/ui-v2` 已合併為此版本。

**設計語彙**：
- **和紙**（淺）暖米白 ×「**墨**」（深）暖炭黑雙主題——皆為暖調，無藍灰
- **朱印紅** primary：像印鑑只落在關鍵處；章節索引為 **hanko 式朱印小方框**（`.seal-index`）
- **明朝體**（Noto Serif TC）h1/h2 主標；黑體次標；mono 數據不變
- 語意色：**苔**綠 / **山吹**黃 / 紅 / **藍**；圖表五色 **朱・藍・苔・山吹・江戶紫**（dataviz 六項檢查雙主題 PASS——原「茶」因彩度不足讀為灰而更換）
- 近直角圓角、標頭去底色（僅 hairline）、字距放寬、版面 padding 加大（余白）
- 結構沿用 v3（分組側欄/帳冊合計列/分錄簿/收件匣/狀態列），LoadingScreen・跑貓框・豹爪撕裂邊全數換裝朱墨

**分支狀態**：`redesign/ui-v2` = 和紙と墨（正式）；`redesign/jp-minimal` = 同內容（可刪）；工程帳冊琥珀版保留於歷史 commit `b5cd31f`，隨時可 checkout 回顧。

---

## ⚡ v3 白紙重構（07-04 追加）

你澄清「完全不需要保留原先任何東西」後，第二輪把**版面結構與資訊架構也全部從零重想**（v2 只換皮保留了舊骨架）。功能佈線與資料流不動，其餘全新：

| 頁面 | v3 全新結構 |
|---|---|
| **Shell** | 分組側欄（`00 總覽 / 01 財務 / 02 營運 / 03 系統` mono 分區碼）、mono 麵包屑 header、**底部工程狀態列**（`● user@role · 組別 · FRC 6998 UNIPARDS`） |
| **儀表板** | 廢除 bento 卡片海 → 期別標頭（`2026 SEASON · 日期`）+ 帳冊合計列（hairline 分欄）+ 左「近期帳目」journal 表格 + 右行動欄（待辦計數、快速動作） |
| **我的花費** | 卡片堆疊 → **分錄簿連續總帳**（分錄標頭列 + `01.1` 式縮排明細子列 + 帳戶尾註） |
| **新增報帳單** | 單欄長表 → **採購單雙欄**（左編號分節表單 + 右 sticky 即時摘要：項目數/總額 live 計算） |
| **審核** | 堆疊卡 → **審核收件匣佇列**（左案件+縮排明細預覽，右操作艙：金額+核准/拒絕），狀態接上正式雙語標籤 |
| **所有報表** | 合計帶 + **分段控制篩選器**與表格一體化登記簿（含筆數指示） |
| **數據分析** | 合計帶 + 編號章節圖表（01 趨勢 / 02 類別 / 03 狀態） |
| **庫存** | **庫房清單一體式容器**（搜尋/篩選工具列併入表格 + `N/總數` 指示） |
| **資金** | **對帳單視圖**（合計帶 + 工具列表格一體） |
| **用戶** | 名冊（合計帶 + 表格） |
| **設定** | 編號章節（01 頭像 ~ 06 帳戶，動態編號） |
| **個人資料** | **隊員檔案卡**（識別列 + mono 定義表 + 統計帶） |
| **About** | 全面重造為**隊伍工程日誌 LOGBOOK**（藍圖網格封面、`LOG-20XX` 分錄、反色哲學帶、mono 社群清單）— 隊史內容與連結全數保留 |

品質：`tsc` 0 錯、`build` 24 頁全過、`lint` 乾淨（僅 2 既有警告）。v3 截圖：`docs/redesign/screenshots/v3/`。
