...
export function InventoryContent({ items, restockItems, userRole }: InventoryContentProps) {
    ...
    const handleAddItem = async () => {
        startTransition(async () => {
            ...
            if (result.success) {
                ...
                showMessage("error", result.message || "操作失敗")
            }
        })
    }
    ...
    const handleDeleteItem = async (itemId: string) => {
        if (!confirm(language === "zh" ? "確定要刪除此物品嗎？" : "Delete this item?")) return
        startTransition(async () => {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        {language === "zh" ? "管理 FRC 物品庫存" : "Manage parts inventory"}
                    </p>
                </div>
                <button
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-card rounded-xl p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">
                                    {language === "zh" ? "安全庫存警戒線" : "Safety Stock Level"}
                                </label>
                                <input
                                    type="number"
