...
const CATEGORIES: { value: ItemCategory; labelZh: string; labelEn: string }[] = [
    { value: "MOTOR", labelZh: "È¶¨È?", labelEn: "Motor" },
    { value: "SENSOR", labelZh: "?üÊ∏¨??, labelEn: "Sensor" },
    { value: "PNEUMATIC", labelZh: "Ê∞??", labelEn: "Pneumatic" },
    { value: "CONTROLLER", labelZh: "?ßÂà∂??, labelEn: "Controller" },
    { value: "HARDWARE", labelZh: "‰∫îÈ?", labelEn: "Hardware" },
    { value: "RAW_MATERIAL", labelZh: "?üÊ?", labelEn: "Raw Material" },
    { value: "TOOL", labelZh: "Â∑•ÂÖ∑", labelEn: "Tool" },
]

const TRANSACTION_TYPES: { value: TransactionType; labelZh: string; labelEn: string }[] = [
    { value: "PURCHASE_IN", labelZh: "?°Ë≥º?•Â∫´", labelEn: "Purchase In" },
    { value: "PROJECT_USE", labelZh: "Â∞àÊ??òÁî®", labelEn: "Project Use" },
    { value: "DAMAGED", labelZh: "?çÂ??±Âª¢", labelEn: "Damaged" },
    { value: "LOST", labelZh: "?∫Â§±", labelEn: "Lost" },
    { value: "AUDIT_ADJUSTMENT", labelZh: "?§È?Ë™øÊï¥", labelEn: "Audit Adjustment" },
]
...
export function InventoryContent({ items, restockItems, userRole }: InventoryContentProps) {
    ...
    const handleAddItem = async () => {
        startTransition(async () => {
            ...
            if (result.success) {
                showMessage("success", result.message || "?êÂ?")
                setShowAddModal(false)
                window.location.reload()
            } else {
                showMessage("error", result.message || "Â§±Ê?")
            }
        })
    }
    ...
    const handleDeleteItem = async (itemId: string) => {
        if (!confirm(language === "zh" ? "Á¢∫Â?Ë¶ÅÂà™?§Ê≠§?∂‰ª∂?éÔ?" : "Delete this item?")) return
        startTransition(async () => {
            const result = await deleteItem(itemId)
            if (result.success) {
                setLocalItems((prev) => prev.filter((i) => i.id !== itemId))
                showMessage("success", result.message || "Â∑≤Âà™??)
            } else {
                showMessage("error", result.message || "Â§±Ê?")
            }
        })
    ...
    const handleQuickStockIn = async () => {
        ...
        startTransition(async () => {
            ...
            if (result.success) {
                showMessage("success", result.message || "?•Â∫´?êÂ?")
    ...
    const handleQuickStockOut = async () => {
        ...
        startTransition(async () => {
            const result = await adjustStock(
                selectedItem.id,
                -adjustData.amount, // Ë≤†Êï∏Ë°®Á§∫?∫Â∫´
                adjustData.type,
                adjustData.projectId || undefined
            )
            if (result.success) {
                showMessage("success", result.message || "?òÁî®?êÂ?")
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">
                        {language === "zh" ? "Â∫´Â?ÁÆ°Á?" : "Inventory Management"}
                    </h1>
                    <p className="text-muted-foreground">
                        {language === "zh" ? "ÁÆ°Á? FRC ?∂‰ª∂Â∫´Â?" : "Manage FRC parts inventory"}
                    </p>
                </div>
                <button
                ...
                <div className="p-4 rounded-xl border border-yellow-200 bg-yellow-50">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="font-semibold text-yellow-800">
                            {language === "zh" ? "?ÄË¶ÅË?Ë≤? : "Need Restock"} ({restockItems.length})
                        </h3>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-1">
                    ...
                    <input
                        type="text"
                        placeholder={language === "zh" ? "?úÂ??ÅÂ??ñÊ???.." : "Search name or SKU..."}
                        value={searchTerm}
                ...
                >
                    <option value="all">{language === "zh" ? "?Ä?âÈ??? : "All Categories"}</option>
                    {CATEGORIES.map((cat) => (
            ...
            <div className="rounded-xl border bg-card overflow-x-auto">
                <table className="w-full">
                    <thead className="bg-muted/50">
                        <tr>
                            <th className="text-left p-4 font-medium">{language === "zh" ? "?ÅÂ?" : "Name"}</th>
                            <th className="text-left p-4 font-medium">{language === "zh" ? "?ôË?" : "SKU"}</th>
                            <th className="text-left p-4 font-medium">{language === "zh" ? "È°ûÂà•" : "Category"}</th>
                            <th className="text-left p-4 font-medium">{language === "zh" ? "‰ΩçÁΩÆ" : "Location"}</th>
                            <th className="text-left p-4 font-medium">{language === "zh" ? "?∏È?" : "Qty"}</th>
                            <th className="text-left p-4 font-medium">{language === "zh" ? "ÂÆâÂÖ®Â∫´Â?" : "Safety"}</th>
                            <th className="text-left p-4 font-medium">{language === "zh" ? "?ç‰?" : "Actions"}</th>
                        </tr>
                    </thead>
                                    ...
                                    <td className="p-4">
                                        <div className="flex gap-1">
                                            ...
                                            <button
                                                ...
                                                title={language === "zh" ? "?òÁî®" : "Stock Out"}
                                            >
                                            ...
                                            <button
                                                ...
                                                title={language === "zh" ? "?≤È?Ë™øÊï¥" : "Advanced"}
                                            >
                                            ...
                                            {item.vendorLink && (
                                                <a
                                                    ...
                                                    className="p-1.5 rounded hover:bg-muted"
                                                    title={language === "zh" ? "Ë≥ºË≤∑???" : "Vendor Link"}
                                                >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-card rounded-xl p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">
                                    {language === "zh" ? "?≤Â?‰ΩçÁΩÆ" : "Storage Location"}
                                </label>
                                <input
                                    type="text"
                            ...
                            {showAddModal && (
                                <div>
                                    <label className="block text-sm font-medium mb-1">
                                        {language === "zh" ? "?ùÂ??∏È?" : "Initial Quantity"}
                                    </label>
                                    <input
                                        type="number"
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">
                                    {language === "zh" ? "ÂÆâÂÖ®Â∫´Â?Ê∞¥‰?" : "Safety Stock Level"}
                                </label>
                                <input
                                    type="number"
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-card rounded-xl p-6 w-full max-w-md mx-4">
                        <h2 className="text-xl font-bold mb-2">
                            {language === "zh" ? "Ë™øÊï¥Â∫´Â?" : "Adjust Stock"}
                        </h2>
                        <p className="text-muted-foreground mb-4">
                            {selectedItem.name} ({language === "zh" ? "?ÆÂ?" : "Current"}: {selectedItem.currentQuantity})
                        </p>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium mb-1">
                                    {language === "zh" ? "?∞Â?È°ûÂ?" : "Transaction Type"}
                                </label>
                                <select
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">
                                    {language === "zh" ? "?∏È? (Ê≠??•Â∫´ÔºåË?=?∫Â∫´)" : "Amount (+in/-out)"}
                                </label>
                                <input
                                    type="number"
                            ...
                            {adjustData.type === "PROJECT_USE" && (
                                <div>
                                    <label className="block text-sm font-medium mb-1">
                                        {language === "zh" ? "Â∞àÊ? ID" : "Project ID"}
                                    </label>
                                    <input
                                        type="text"
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-card rounded-xl p-6 w-full max-w-sm mx-4">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium mb-1">
                                    {language === "zh" ? "?•Â∫´?∏È?" : "Quantity to Add"}
                                </label>
                                <input
                                    type="number"
                                    min="1"
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-card rounded-xl p-6 w-full max-w-sm mx-4">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium mb-1">
                                    {language === "zh" ? "?òÁî®?∏È?" : "Quantity to Take"}
                                </label>
                                <input
                                    type="number"
                                    min="1"
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">
                                    {language === "zh" ? "?®ÈÄ?Â∞àÊ?" : "Purpose/Project"}
                                </label>
                                <input
                                    type="text"
                                    value={adjustData.projectId}
                                    onChange={(e) => setAdjustData({ ...adjustData, projectId: e.target.value })}
                                    placeholder={language === "zh" ? "‰æãÂ?: 2024 Ê©üÂô®‰∫? : "e.g. 2024 Robot"}
                                    className="w-full px-3 py-2 border rounded-lg"
                                />
                            </div>
                        </div>
