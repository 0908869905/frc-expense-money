...
export function DepartmentBudgetContent({ departmentSummary, userRole, userDepartment }: DepartmentBudgetContentProps) {
    ...
    const handleSave = async (dept: string) => {
        startTransition(async () => {
            try {
                ...
                showMessage("success", t("?êÁ?Â∑≤Êõ¥??, "Budget updated"))
                setEditingDept(null)
                // Force refresh
    ...
    const visibleDepartments = DEPARTMENTS.filter(dept => {
        // Ë≤°Â??åÁÆ°?ÜÂì°?ãÂà∞?Ä?âÁ???
        if (canEdit) return true
        // Â¶ÇÊ??®Êà∂?âÊ?ÂÆöÁ??•Ô??™Á??∞Ëá™Â∑±Á?ÁµÑÂà•
        if (userDepartment) {
            return dept.value === userDepartment
        }
        // Ê≤íÊ??áÂ?ÁµÑÂà•??LEADER/VICE_LEADER ?ãÂà∞?Ä?âÁ???
        return true
    })
                        ...
                        {overspentDepts.map(dept => {
                            ...
                            return (
                                <p key={dept.value} className="text-sm text-red-600">
                                    {dept.icon} {dept[language as "zh" | "en"]}Ôº?
                                    {t("Ë∂ÖÊîØ ", "Overspent by ")}
                        ...
                        >
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between text-sm">
                                    <span className="text-muted-foreground">{t("?êÁ?", "Budget")}</span>
                                ...
                                <div className="flex justify-between text-sm">
                                    <span className="text-muted-foreground">{t("Â∑≤Áî®", "Spent")}</span>
                                    <span className="font-medium">{formatCurrency(data.spent)}</span>
                                </div>

                                <div className="flex justify-between text-sm">
                                    <span className="text-muted-foreground">{t("?©È?", "Remaining")}</span>
                            ...
                            <div className="mt-3">
                                ...
                                <p className="text-xs text-muted-foreground mt-1 text-right">
                                    {progressPercent.toFixed(0)}% {t("Â∑≤‰Ωø??, "used")}
                                </p>
                            </div>
                        </div>
                    )
