...
export function ApprovalsContent({ reports, userRole }: ApprovalsContentProps) {
    ...
    const handleReject = async (reportId: string) => {
        const reason = prompt(language === "zh" ? "Ë´ãËº∏?•Ê?ÁµïÂ??†Ô?" : "Please enter rejection reason:")
        if (!reason) return
        ...
        <div className="flex flex-col gap-6">
            <div>
                <h1 className="text-3xl font-bold tracking-tight">
                    {language === "zh" ? "ÂØ©Ê†∏?±Â∏≥?? : "Approvals"}
                </h1>
                <p className="text-muted-foreground">
                    {language === "zh"
                        ? `‰Ω†Ê? ${localReports.length} ?ãÂ?ÂØ©Ê†∏?ÑÂ†±Â∏≥ÂñÆ`
            ...
            <div className="space-y-4">
                {localReports.length === 0 ? (
                    <div className="rounded-xl border bg-card p-8 text-center">
                        ...
                        <p className="text-muted-foreground">
                            {language === "zh" ? "Ê≤íÊ?ÂæÖÂØ©?∏Á??±Â∏≥?? : "No pending expense reports"}
                        </p>
                    </div>
                ) : (
                        ...
                        <div key={report.id} className="rounded-xl border bg-card overflow-hidden">
                            <div className="p-4 border-b bg-muted/30 flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        {language === "zh" ? "?ê‰∫§?? : "Submitted by"}: {report.submitter?.name || report.submitter?.email}
                                    </p>
                                    <p className="text-sm text-muted-foreground">
                                        {formatDate(report.createdAt)} ??{report.items.length} {language === "zh" ? "Á≠ÜÈ??? : "items"}
                                    </p>
                                </div>
                            ...
                            <div className="p-4 border-b">
                                <p className="text-sm font-medium mb-2">
                                    {language === "zh" ? "Ë≤ªÁî®?éÁ¥∞" : "Expense Items"}:
                                </p>
                                <div className="space-y-1">
                                    ...
                                    {report.items.length > 3 && (
                                        <p className="text-xs text-muted-foreground">
                                            +{report.items.length - 3} {language === "zh" ? "?¥Â??ÖÁõÆ" : "more items"}
                                        </p>
                                    )}
                                </div>
                            </div>
