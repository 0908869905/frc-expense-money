...
export function BalanceCard({ totalIncome, totalExpense, currentBalance, fundingRecords = [] }: BalanceCardProps) {
    ...
    const handleDelete = (id: string) => {
        if (!confirm("Á¢∫Â?Ë¶ÅÂà™?§Ê≠§Ë≥áÈ?Ë®òÈ??éÔ?")) return
        startTransition(async () => {
            ...
            if (result.success) {
                ...
                showMessage("error", result.message || "?™Èô§Â§±Ê?")
            }
        })
    }
    ...
    const handleUpdate = () => {
        ...
        startTransition(async () => {
            ...
            if (result.success) {
                showMessage("success", result.message || "Â∑≤Êõ¥??)
                setEditingRecord(null)
                window.location.reload()
            } else {
                showMessage("error", result.message || "?¥Êñ∞Â§±Ê?")
            }
        })
    }
    ...
    return (
        <>
            <div className="rounded-xl border bg-gradient-to-br from-primary/10 via-background to-background p-6">
                ...
                {/* ?ÆÂ?È§òÈ? - Â§ßÊ?È°?*/}
                <div className="mb-6">
                    <p className="text-sm text-muted-foreground mb-1">?ÆÂ?È§òÈ?</p>
                    <p
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-xs text-muted-foreground">Á∏ΩÊî∂??/p>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-xs text-muted-foreground">Á∏ΩÊîØ??/p>
                ...
                {expanded && fundingRecords.length > 0 && (
                    <div className="mt-6 pt-4 border-t">
                        <h4 className="text-sm font-medium mb-3">Ë≥áÈ?Ë®òÈ?</h4>
                        {message && (
                                ...
                                >
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            {formatDate(record.date)} ??{record.source || "?™Áü•‰æÜÊ?"} ??{record.recordedBy}
                                        </p>
                                    </div>
                    ...
                    <div className="bg-background rounded-xl border shadow-lg w-full max-w-md mx-4">
                        ...
                        <div className="p-4 space-y-4">
                            <div className="space-y-2">
                                <Label htmlFor="edit-title">Ê®ôÈ?</Label>
                                <Input
                                    id="edit-title"
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="edit-amount">?ëÈ?</Label>
                                <Input
                                    id="edit-amount"
                                    type="number"
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="edit-type">È°ûÂ?</Label>
                                <select
                                    id="edit-type"
                                    value={editForm.type}
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="edit-source">‰æÜÊ?</Label>
                                <Input
                                    id="edit-source"
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="edit-date">?•Ê?</Label>
                                <Input
                                    id="edit-date"
                                    type="date"
                                    value={editForm.date}
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="edit-description">?ôË®ª</Label>
                                <textarea
                                    id="edit-description"
